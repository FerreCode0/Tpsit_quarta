...
Student** loadStudentsFromFile(FILE* f, int* outCount); //vettore di puntatore
...
Student** loadStudentsFromFile(FILE* f, int* outCount) {
    ...
    while((line = readLine(f)) != NULL) { //readline fà la malloc
...
//riveo un file star e un vett di puntatori a studente, scorre e per ogni studente scrive una riga 
void saveStudentsToFile(FILE* file, Student** students) {
    for(Student** temp = students; *temp; temp++) {
...
void disposeStudentArray(Student** students) {
    for(Student** temp = students; *temp; temp++) {
        //ammazza il nome e poi lo studente
        disposeStudent(*temp);
    }
    free(students);
}
